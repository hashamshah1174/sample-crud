...
function ProductModal() {
    ...
    const handleShow = () => {
        axios.get('http://localhost:1190/api/get-all-brands').then(json => {
            setBrandVal(json.data.brands)
            axios.get('http://localhost:1190/api/all-categories').then(json => {
                ...
                uploadBytes(storageRef, thumbnail).then((snapshot) => {
                    getDownloadURL(snapshot.ref)
                        .then((url) => {
                            ...
                            axios.post('http://localhost:1190/api/add-products', payload).then((json) => {
